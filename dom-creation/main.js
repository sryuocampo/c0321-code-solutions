...
function renderPokemon(pokemon) {
  ...
  var numberItem = document.createElement('h3');
  ...
  divPCT.setAttribute('class', 'pokemon-card-text');

  divPCT.append(nameItem, numberItem, descriptionItem);

  var image = document.createElement('img');
  image.src = pokemon.imageUrl;

  var divPC = document.createElement('div');
  divPC.setAttribute('class', 'pokemon-card');

  divPC.append(image, divPCT);

  var divColumnThrid = document.createElement('div');
  divColumnThrid.setAttribute('class', 'column-third');

  divColumnThrid.append(divPC);

  return divColumnThrid;
}

var rowElement = document.querySelector('.row');

for (var i = 0; i < pokedex.length; i++) {
  var el = renderPokemon(pokedex[i]);
